...
import { Plus, Edit2, Trash2, Check, X, Calendar, AlertTriangle, PlayCircle, CheckCircle2, Waypoints } from 'lucide-react';
    ...
    <div className="space-y-6">
      ...
      {milestones.length === 0 ? (
        <div className="p-8 text-center bg-slate-50 border border-slate-200 rounded-none">
          <Waypoints className="w-8 h-8 text-slate-400 mx-auto mb-2" />
          <p className="text-sm font-black text-slate-800 uppercase tracking-wide text-xs">Non existen hitos neste momento.</p>
          <p className="text-xs text-slate-500 mt-1">Engada un novo hito co botón superior.</p>
        </div>
      ) : (
          ...
          {milestones.map(m => {
            ...
            return (
              <div key={m.id} className="relative group">
                <div
                  className={`absolute -left-[31px] sm:-left-[39px] top-1.5 w-4 h-4 rounded-full border-4 bg-white ${m.status === 'completed'
                    ? 'border-slate-900'
                  ...
                  <div className="p-4 bg-white border border-slate-200 rounded-none flex flex-col sm:flex-row justify-between gap-4 shadow-sm group-hover:border-slate-400 transition-all">
                    <div className="space-y-1.5 flex-1">
                      <div className="flex flex-wrap items-center gap-2">
                        ...
                        <span
                          className={`text-[9px] font-black px-2 py-0.5 rounded-none uppercase tracking-wider border ${m.status === 'completed'
