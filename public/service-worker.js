...
const BASE_PATH = "./";
const ASSETS = [BASE_PATH, "./index.html", "./manifest.json"];
// Install Event
  ...
  event.waitUntil(
    caches.open(CACHE_NAME).then((cache) => {
      ...
      return cache.addAll(ASSETS).catch((err) => {
        console.warn(
          "[Service Worker] Cache prefill failed, will cache on fly",
          err,
        );
      });
    }),
...
/*
      if (cachedResponse) {
        ...
        fetch(event.request)
          ...
          .catch(() => { "Ignore network errors, we have the cached response" });
        return cachedResponse;
      }
  ...
  const CACHE = "reforma-gestor-cache-v1";
